...
public class ListUsersAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        NSLog.i("viewType", viewType + "");

        View rootView = null;
        if (isHorizontalList) rootView = infalter.inflate(R.layout.v3_item_user, parent, false);
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder mHolder, final int position) {
        ...
        if (AppConfig.APP_DEBUG)
            NSLog.i("user", position + " " + data.get(position).getUsername());


        NSLog.i("distance - " + data.get(position).getUsername() + " - " + data.get(position).getType(), String.valueOf(data.get(position).getDistance()));
    ...
    public class mViewHolder extends RecyclerView.ViewHolder {
        ...
        public mViewHolder(View itemView) {
            ...
            optionIcon.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if (clickListener != null) {
                        clickListener.itemOptionsClicked(optionIcon, getLayoutPosition());
                    }
                }
            });
